...
import com.clientScript.exception.ResolvedException;
...
public class Expression {
    public static FabricClientCommandSource source = null;
    ...
    public static final Expression none = new Expression("null", Expression.source);
    ...
    public Expression(String expression, FabricClientCommandSource source) {
    ...
    public void asModule(Module mi) {
        this.module = mi;
    }
    ...
    public String getModuleName() {
        return this.module == null ? "system chat" : module.getName();
    }
    ...
    public Value eval(Context c) {
        if (this.ast == null)
            this.ast = getAST(c);
        return evalValue(() -> this.ast, c, Context.Type.NONE);
    }
...
//        if (!CarpetSettings.scriptsOptimization)  // TODO remove comment
//            return root.op;

        Context optimizeOnlyContext = new Context.ContextForErrorReporting(context);
//        if (CarpetSettings.scriptsDebugging)  // TODO remove comment
//            CarpetScriptServer.LOG.info("Input code size for "+getModuleName()+": " + treeSize(root) + " nodes, " + treeDepth(root) + " deep");
        ...
        while(changed) {
            changed = false;
            while (true) {
//                int tree_size = treeSize(root);  // TODO remove comment
        ...
        for (Tokenizer.Token token : cleanedTokens) {
            switch (token.type) {
                ...
                case OPERATOR: {
                    ...
                    Fluff.ILazyOperator o1 = operators.get(token.surface);
                    if (o1 == null)
        ...
        for (final Tokenizer.Token token : tokens) {
            switch (token.type) {
                ...
                case FUNCTION:
                    ...
                    boolean isKnown = this.functions.containsKey(name); // globals will be evaluated lazily, not at compile time via .
                    if (isKnown) {
                    ...
                    nodeStack.push(new ExpressionNode(
                        (c, t) -> f.lazyEval(c, t, this, token, params).evalValue(c, t), p, token)
                    );
                    break;
                case OPEN_PAREN:
...
*/
    private boolean compactTree(ExpressionNode node, Context.Type expectedType, int indent) {
        ...
        for (ExpressionNode arg : node.args) {
            if (compactTree(arg, requestedType, indent + 1))
                optimized = true;
        }
...
//                        CarpetScriptServer.LOG.info(" - Removed unnecessary tail return from function body at line " + (returnNode.token.lineno + 1) + ", node depth " + indent);
        ...
        for (Map.Entry<String, String> pair : this.functionalEquivalence.entrySet()) {
...
//                        CarpetScriptServer.LOG.info(" - " + symbol + "(" + node.args.size() + ") => " + function + "(" + newargs.size() + ") at line " + (node.token.lineno + 1) + ", node depth " + indent);
    ...
    private boolean optimizeTree(Context ctx, ExpressionNode node, Context.Type expectedType, int indent) {
        ...
        Context.Type requestedType = operation.staticType(expectedType);
        for (ExpressionNode arg : node.args)
            if (optimizeTree(ctx, arg, requestedType, indent + 1))
                optimized = true;
        ...
        if (operation instanceof Fluff.ILazyFunction)
            result = ((Fluff.ILazyFunction)operation).lazyEval(ctx, expectedType, this, node.token, args).evalValue(null, expectedType);
        else if (args.size() == 1)
            result = ((Fluff.ILazyOperator)operation).lazyEval(ctx, expectedType, this, node.token, args.get(0), null).evalValue(null, expectedType);
        else // args == 2
            result = ((Fluff.ILazyOperator)operation).lazyEval(ctx, expectedType, this, node.token, args.get(0), args.get(1)).evalValue(null, expectedType);
...
//            CarpetScriptServer.LOG.info(" - "+symbol+"("+args.stream().map(a -> a.evalValue(null, requestedType).getString()).collect(Collectors.joining(", "))+") => "+result.getString()+" at line "+(node.token.lineno+1) +", node depth "+indent);
    ...
    public void addLazyBinaryOperator(String surface, int precedence, boolean leftAssoc, boolean pure, Function<Context.Type, Context.Type> typer, Fluff.QuadFunction<Context, Context.Type, LazyValue, LazyValue, LazyValue> lazyfun) {
    ...
    public void addPureLazyFunction(String name, int num_params, Function<Context.Type, Context.Type> typer, Fluff.TriFunction<Context, Context.Type, List<LazyValue>, LazyValue> fun) {
    ...
    public void addLazyUnaryOperator(String surface, int precedence, boolean leftAssoc, boolean pure, Function<Context.Type,Context.Type> staticTyper, Fluff.TriFunction<Context, Context.Type, LazyValue, LazyValue> lazyfun) {
    ...
    public void addLazyFunction(String name, int num_params, Fluff.TriFunction<Context, Context.Type, List<LazyValue>, LazyValue> fun) {
        this.functions.put(name, new Fluff.AbstractLazyFunction(num_params, name) {
            @Override
            public boolean pure() {
                return false;
    ...
    public void addBinaryContextOperator(String surface, int precedence, boolean leftAssoc, boolean pure, boolean transitive, Fluff.QuadFunction<Context, Context.Type, Value, Value, Value> fun) {
        this.operators.put(surface, new Fluff.AbstractLazyOperator(precedence, leftAssoc) {
            @Override
            public boolean pure() {
                return pure;
    ...
    public void addContextFunction(String name, int num_params, Fluff.TriFunction<Context, Context.Type, List<Value>, Value> fun) {
        this.functions.put(name, new Fluff.AbstractLazyFunction(num_params, name) {
            @Override
            public boolean pure() {
                return false;
    ...
    public void addLazyBinaryOperatorWithDelegation(String surface, int precedence, boolean leftAssoc, boolean pure, Fluff.SexFunction<Context, Context.Type, Expression, Tokenizer.Token, LazyValue, LazyValue, LazyValue> lazyfun) {
        this.operators.put(surface, new Fluff.AbstractLazyOperator(precedence, leftAssoc) {
            @Override
            public boolean pure() {
                return pure;
    ...
    public void addTypedContextFunction(String name, int num_params, Context.Type reqType, Fluff.TriFunction<Context, Context.Type, List<Value>, Value> fun) {
        this.functions.put(name, new Fluff.AbstractLazyFunction(num_params, name) {
            @Override
            public boolean pure() {
                return true;
    ...
    public void addMathematicalBinaryFunction(String name, BiFunction<Double, Double, Double> fun) {
        addBinaryFunction(name, (w, v) -> new NumericValue(fun.apply(NumericValue.asNumber(w).getDouble(), NumericValue.asNumber(v).getDouble())));
    }

    public void addFunctionWithDelegation(String name, int numpar, boolean pure, boolean transitive, Fluff.QuinnFunction<Context, Context.Type, Expression, Tokenizer.Token, List<Value>, Value> fun) {
